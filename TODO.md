...
| R-4 | `test-systems-harness` "DB cast spark" check is non-deterministic, not flaky in the random/hardware sense — it creates two test kingdoms at random map positions each run with no elevation control, and `game/magic.js:134-142`'s real `FEATURE_ELEVATION_SPELLS` line-of-sight gate legitimately rejects casts when the random pair lands at different elevations. The test's `assertOk` doesn't accept that legitimate outcome, so it fails whenever placement happens to trigger the gate. **Elevation is explicitly out of scope this session** (user directive, see memory) — not fixed; flagged instead of silently re-labeled "flaky" a second time. Fix is test-side only (force same-elevation seed kingdoms, or accept the elevation-blocked outcome as pass) and doesn't require touching elevation feature logic, but confirm with user before touching anything elevation-adjacent. | **TODO — flagged, not fixed** |
