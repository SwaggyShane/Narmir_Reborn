...
| A2-3 | Split gameplay — **first slice:** turn + turn helpers only → e.g. `routes/kingdom-turn.js`, mount in `kingdom.js` before/after agreed position. | **INVESTIGATED, not done — genuinely bigger than scoped.** Real dependency map (2026-07-19): `withTurnLock` (142), `loadTurnContext` (298), `commitTurnResults` (333-524), `processTurnEffectsOnly` (527 — **dead, zero callers anywhere, safe to delete separately**), `runTurn` (595), `POST /turn` (642-786) are the turn slice — but `commitTurnResults` depends on `applyUpdates`/`bulkInsertNews`/`pruneNews` (defined at 3658-3729, used by many *other*, non-turn routes in this file) and `getRandomKingdom`/`repairMojibake`/`normalizeNewsRow` (defined near the top, 40-142, likely also used elsewhere). `runTurn` itself is also called by `/smithy/forge-tools` and `/search`, which are not moving. A correct extraction needs the shared helpers pulled into their own module first (so `kingdom-turn.js` and `kingdom-gameplay.js` can both use them without a circular require between the two) — that's separate groundwork, not part of "just move the turn route." This is the single most-called code path in the game; not doing it rushed. Also confirmed dead and unexported-externally: `router.withTurnLock`/`.runTurn`/`.processTurnEffectsOnly`/`.loadTurnContext`/`.commitTurnResults` (lines 3649-3653) — nothing outside this file reads any of them (`grep` across the whole repo), safe to delete once the real extraction happens. |
