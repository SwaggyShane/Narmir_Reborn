...
**Last updated:** 2026-07-19 (TODO.md implementation session: boot cleanup, full kingdom-route + admin-route split series, turn pipeline honesty pass, client response-contract fixes, mutator boundary matrix)

---

## Recent Chronology

### 2026-07-19 — TODO.md implementation session (A1–A5 series, sessioned-on implementation)

**Scope:** A read-only assessment (2026-07-18) had mapped five areas (boot/index.js, kingdom routes, turn pipeline, client state path, CommandHandler boundary) into numbered work items. The user reviewed it, confirmed understanding, then authorized full-autonomy implementation with an explicit standing bar: "test, document, no path of least resistance, fix it right, don't ignore warnings." Everything below was executed under that bar — every item went through the full local acceptance baseline (lint, `architecture:accept`, `check:command-boundary`, `validate:game-tables`, `npm test`, `npm run test:systems`) plus live HTTP verification against the real running dev server, then a feature-branch → merge to local `main` cycle (local-only per the TODO's own declared mode — no PRs). Full narrative detail for every item lives in this session's commit messages; this entry is the condensed summary. Items not listed here (A3-2/A3-5–8, A4-3–8, A5-4/5/8) remain open — see `TODO.md`.

**§1 Boot / `index.js` (A1-1…A1-8) — fully complete.** `index.js` collapsed from 172 to 37 lines. Consolidated the two duplicate `unhandledRejection`/`uncaughtException` registrations into one owner (`lib/error-handlers.js`), extracted `lib/shutdown.js` for a single graceful-shutdown path, `lib/bootstrap.js` for the `start()` body, collapsed `setupRoutes`'s 14-property DI bag to 3 (`db`, `io`, `getBootError`). Fixed a real bug while extracting shutdown: the original code cleared the 10-second force-exit safety timer inside its own catch block, silently disabling the one path that needed it. Added an `architecture:accept` check that fails if `index.js` exceeds 60 lines. `gracefulShutdown` verified directly (not just reviewed) across all three paths — happy, thrown-error, genuine-hang.

**§2 Kingdom + admin route splits (A2-1…A2-10) — fully complete.** `kingdom-gameplay.js` (originally 53 handlers, ~3346 lines — the "junk drawer" for everything that never got its own router) split into 6 files across five sequential passes: `kingdom-turn.js` (turn + shared helpers extracted to `routes/lib/kingdom-turn-helpers.js`), `kingdom-forge.js` (Forge & Lava Industry — verified as one real economic loop, not just naming-adjacent), `kingdom-prestige.js` (Prestige + Dragon Evolution — verified real coupling: evolution reads `prestige_level` as its unlock gate), `kingdom-attunements.js` (Inventory/Attunements/Synergies — verified attunements and synergies share `fragment_bonuses` state), `kingdom-worldmap.js` (World Map/Locations/Rivers/Scout-debug — 4 non-contiguous blocks, verified as layers of one visibility system), and `kingdom-social.js` (News/Chat/Scouts/Portrait/Happiness — the true remainder). Every extraction was byte-diffed against the original before deletion; every split correctly excluded look-alike routes that turned out to be different systems on inspection (`/smithy/forge-tools` vs the real Forge system; `GET /profile/:name` vs world-map; `GET /scouts` vs `/debug/scouts`). `kingdom-gameplay.js` ended at 17 handlers.

`routes/admin.js` (2363 lines — 68 direct routes + 16 more hidden behind an internal `dualRoute()` legacy-alias helper a naive grep missed entirely) split into 7 domain files (`admin-kingdoms.js`, `admin-ai.js`, `admin-events.js`, `admin-lore.js`, `admin-goals.js`, `admin-config.js`, `admin-audit.js`) plus two shared modules (`routes/lib/admin-kingdom-helpers.js` for reset/seed logic genuinely shared between the kingdoms and AI clusters; `routes/lib/admin-dual-route.js`). `admin.js` itself became a thin composer mirroring `kingdom.js`'s pattern — necessary because the original applied `requireAdmin`+CSRF via router-level `router.use(...)`, not per-route, so the composer applies that gate exactly once before mounting any sub-router rather than risk it being silently omitted from a future file. Extraction used a small one-off Node script (statement-boundary parsing via brace/paren depth counting) given the scale, not manual line-hunting; verified via `eslint` (zero missing/unused-import warnings across every new file) and live HTTP checks confirming the auth gate composition: no-auth → 401, non-admin token → 403, valid admin token → 200 across all 7 files including legacy aliases.

Found and fixed three real bugs during this series, all unrelated to the splits themselves but caught during live verification: a `GET /inventory` crash on kingdom id=1 from legacy numeric-shaped item data (`item.name.includes` on a number); `test-systems-harness`'s route scanner being completely blind to `dualRoute()`-registered paths (fixed the scanner, not just worked around it — scanned handler count went 120→288); and a `test-systems-harness` spell-cast check that had been mischaracterized as "flaky" when it was actually a deterministic elevation-gated rejection the test didn't handle (fixed with a new `AcceptableOutcome` mechanism in the harness, distinct from PASS/FAIL). `kingdom.js`'s mount order was formalized as the sole source of truth (A2-10), with the actual ordering rationale documented, not just the order itself; A2-2's dynamic-path shadowing concern was checked for real (3 dynamic routes, no collisions) rather than assumed covered by the exact-string dup scan.

**§3 Turn pipeline (A3-1, A3-3, A3-4) — partial, real numbers replace estimates.** `game/TURN_PIPELINE.md` was full of fabricated "Estimated time" ranges never actually measured (attunements guessed at 50-200ms). Rewrote it from real data: 8 direct `processTurn` calls against the largest kingdom in the local DB, wrapped in the existing `TurnProfiler`, measured steady-state `totalTime` at ~4.84ms with all 18 attunements individually timed at 0.09–0.23ms each (~2.1ms combined) — the old estimate was off by roughly two orders of magnitude. This also resolved the historical "prod latency ~3-4s" concern (2026-07-08 report, not reproduced since) into a documented guardrail rather than an open investigation: local compute is now ruled out as the cause if it recurs.

**§4 Client state path (A4-1, A4-2, A4-9, A4-10) — partial.** Rebuilt `routes/response-structurer.js` from each Zustand store's actual `receiveServerSnapshot` implementation rather than trusting the file's own "all endpoints MUST use this" claim (which was aspirational, not enforced). Found a bigger bug than "missing fields": `militaryStore` expected a nested `{troops: {...}}` shape but the structurer emitted flat keys, silently no-op'ing troop-count updates on the client. Also fixed dual-domain fields, zero `bld_*` columns being mapped at all, and a full duplicate/more-incomplete copy of the same field lists inline in the `/turn` handler. Added a dev-mode loud-warn for any key that matches no domain, replacing a previous silent drop.

**§5 Mutator boundary / CommandHandler (A5-1, A5-2, A5-3, A5-6, A5-7) — partial.** Classified all 83 mutating kingdom+hero routes into `game/COMMAND_COVERAGE.md` (13 via CommandHandler, 70 via domain module + route transaction) and wrote the policy explaining why that split is intentional, not a gap (Policy B: CommandHandler owns classic engine-rooted systems; newer modularized systems like Forge/Evolution/Prestige/attunements use their own dedicated module). Deleted the dead `process-build-queue` command type (0 callers, and would have thrown a `TypeError` if ever called — wrong argument type passed). Fixed `game/ARCHITECTURE.md`'s false "every route calls engine.processTurn" claim. Confirmed the socket-boundary gap (`game/sockets.js` calls `engine.resolveMilitaryAttack`/`castSpell` directly, unscanned by `check:command-boundary`) is real and still open — A5-4/A5-5, not touched this pass since it touches live combat resolution and needs its own dedicated pass.

**Also fixed:** two instances of stating an unverified claim as fact were caught and corrected mid-series (a data-corruption "pre-existing, not my session" claim that overstated what a code-diff actually proves; the elevation-gated-spell mischaracterization above) — both logged as durable process feedback, not just fixed in the moment.
