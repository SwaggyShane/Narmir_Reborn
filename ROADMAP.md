...
| **F8** | `kingdom.js` split → `build`, `warfare`, `economy`, `research` modules | Incremental refactor; enabled by F3 foundation | Post-F4 | 🟢 **PHASE 1 COMPLETE (PR #646)** — `kingdom-build.js` extracted (16 endpoints: /build-queue, /build-allocation, /resource-build-allocation, /school-allocation, /demolish, /build, /cancel-building, /smithy/*, /tower-*, /shrine-allocation, /mausoleum-allocation, /buy-mausoleum-upgrade, /training-allocation); All endpoints secured with transactions + FOR UPDATE + input validation; Gemini review addressed |
