...
| **F8** | `kingdom.js` split → `build`, `warfare`, `economy`, `research` modules | Incremental refactor; enabled by F3 foundation | Post-F4 | 🟢 **PHASE 1 COMPLETE (PR #646)** — `kingdom-build.js` extracted (16 endpoints); All secured<br/>🟢 **PHASE 2 COMPLETE (PR #647)** — `kingdom-warfare.js` extracted (10 endpoints); /covert, /fire, /defense, /spy-reports secured with FOR UPDATE inside transactions<br/>⚠️ **PHASE 2b IN REVIEW (PR #649)** — /attack and /spell race condition fix attempted (FOR UPDATE added to queries). Code review feedback (Gemini): FOR UPDATE without active transaction doesn't work; requires manual BEGIN/COMMIT/ROLLBACK pattern with sorted locking (like /covert). **DECISION POINT**: (1) Complete full refactor (~500 lines) now using /covert pattern, or (2) Defer Phase 2b and proceed to Phase 3 (remove duplicates)?<br/>⏳ **PHASE 3 PENDING** — Remove duplicate warfare endpoints from kingdom.js |
