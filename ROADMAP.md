...
**Status:** Alpha phase (ongoing) — Tracks A–D complete; E1–E2 fixed; E3 deferred (await discord.js v15); Track F.3 consolidation complete (PR #606–#608); **F.4 engine.js decomposition COMPLETE (PR #611); F.2 Combat V2 complete and alpha-ready (PR #612); F.5 Phase 1 ALL 16 COMPONENTS COMPLETE (PR #617–#641); F.6 Phase 1 COMPLETE (PR #642): Vitest + RTL infrastructure with 57 tests (panelMeta + BottomNav); F.7 Phase 1–3 COMPLETE (PR #643–#645): Numeric range validation utility + full endpoint integration; F.8 Phase 2b–5 COMPLETE (PR #649): Transaction safety for /attack & /spell; removed duplicate warfare endpoints; extracted economy and research modules**  
**Last updated:** 2026-06-28 (F8 Phase 2b–5 COMPLETE: Full transaction-based concurrency; removed 10 duplicate warfare endpoints; extracted 17 economy endpoints; extracted 6 research endpoints; kingdom.js reduced 5,942 → 3,590 lines; lint 0 errors, smoke passing, sanity verified)
...
| **F8** | `kingdom.js` split → `build`, `warfare`, `economy`, `research` modules | Incremental refactor; enabled by F3 foundation | Post-F4 | 🟢 **PHASE 1 COMPLETE (PR #646)** — `kingdom-build.js` extracted (16 endpoints); All secured<br/>🟢 **PHASE 2 COMPLETE (PR #647)** — `kingdom-warfare.js` extracted (10 endpoints); /covert, /fire, /defense, /spy-reports secured with FOR UPDATE inside transactions<br/>✅ **PHASE 2b COMPLETE (PR #649)** — Full transaction-based race condition prevention: /attack and /spell refactored to use manual BEGIN TRANSACTION with sorted kingdom locking (ascending ID order), matching /covert pattern; all validations inside transaction with explicit ROLLBACK on errors; lint 0 errors, smoke passing, sanity verified; Gemini reviews resolved<br/>✅ **PHASE 3 COMPLETE (PR #649)** — Removed 10 duplicate warfare endpoints from kingdom.js (1,037 lines removed); removed unused withTransaction and KINGDOM_COVERT constants; file reduced from 5,942 to 4,819 lines; lint 0 errors, smoke passing, sanity verified<br/>✅ **PHASE 4 COMPLETE (PR #649)** — Extracted 17 economy endpoints to `kingdom-economy.js` (894 lines): /market/*, /trade-routes/*, /economy/*, /trade/clear-logs; moved MARKET_LIQUIDITY constant and dependencies; removed unused marketPriceCache import from kingdom.js; file reduced to 3,963 lines; mounted in index.js with correct precedence; lint 0 errors, smoke passing, sanity verified<br/>✅ **PHASE 5 COMPLETE (PR #649)** — Extracted 6 research endpoints to `kingdom-research.js` (410 lines): /research-allocation, /research, /school-allocation, /research-focus, /studies/overview, /select-school; moved helper functions (repairMojibake, bulkInsertNews) and loadTradeRoutes import; file reduced to 3,590 lines; mounted in index.js with correct precedence; lint 0 errors, smoke passing, sanity verified |
