...
| **F8** | `kingdom.js` split → `build`, `warfare`, `economy`, `research` modules | Incremental refactor; enabled by F3 foundation | Post-F4 | 🟢 **PHASE 1 COMPLETE (PR #646)** — `kingdom-build.js` extracted (16 endpoints); All secured with transactions + FOR UPDATE + input validation<br/>🟢 **PHASE 2 COMPLETE (PR #647)** — `kingdom-warfare.js` extracted (10 endpoints: /war-log, /attack, /spell, /covert, /fire, /defense/overview, /spy-reports); All secured with transactions + FOR UPDATE; Gemini issues fixed<br/>⏳ **PHASE 3 PENDING** — Remove duplicate warfare endpoints from kingdom.js after verification |
