...
module.exports = function (db) {
  ...
  router.post("/attack", requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      ...
      // Fetch attacker ID first (no lock) to enable sorted locking
      const kId = await db.get("SELECT id FROM kingdoms WHERE player_id = ?", [
        req.player.playerId,
      ]);
      if (!kId) {
      ...
      // Lock both kingdoms in sorted order to prevent deadlock
      const kingdomIds = [kId.id, targetId].sort((a, b) => a - b);
      ...
      const k = lockedKingdoms.find(kd => kd.id === kId.id);
      ...
      // All validations inside transaction
      if (k.turns_stored < 1) {
        await db.run("ROLLBACK");
      ...
      if (sentUnits.warMachines > engine.getAvailableUnits(k, "war_machines")) {
        await db.run("ROLLBACK");
        return res.status(400).json({
          error: "Not enough available war machines",
        });
      }
      ...
      if (!atkDisc[targetId] || !atkDisc[targetId].mapped) {
        await db.run("ROLLBACK");
        return res.status(400).json({ error: "You need a location map for this target." });
      }

      // Fetch heroes inside transaction
      ...
      console.error("[attack] operation failed:", err.message);
      return res.status(500).json({ error: "Attack failed - please try again" });
    }
  });
  ...
  router.post("/spell", requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      ...
      // Fetch caster ID first (no lock) to enable sorted locking
      const kId = await db.get("SELECT id FROM kingdoms WHERE player_id = ?", [
        req.player.playerId,
      ]);
      if (!kId) {
      ...
      if (targetId && targetId != kId.id) {
        // Lock both kingdoms in sorted order to prevent deadlock
        kingdomIds = [kId.id, targetId].sort((a, b) => a - b);
        lockedKingdoms = await db.all(
          `SELECT * FROM kingdoms WHERE id IN (${kingdomIds.map(() => '?').join(',')}) ORDER BY id FOR UPDATE`,
          kingdomIds,
        );
        ...
        // Self-cast: only lock caster
        lockedKingdoms = await db.all(
          `SELECT * FROM kingdoms WHERE id = ? FOR UPDATE`,
          [kId.id],
        );

        if (lockedKingdoms.length < 1) {
          await db.run("ROLLBACK");
          return res.status(404).json({ error: "Kingdom not found" });
        }
      } else {
        // Offensive spell requires target
        await db.run("ROLLBACK");
      ...
      const target = isFriendlySpell && !targetId ? k : lockedKingdoms.find(kd => kd.id === targetId);
      if (!target) {
        await db.run("ROLLBACK");
      ...
      console.error("[spell] operation failed:", err.message);
      return res.status(500).json({ error: "Spell cast failed - please try again" });
    }
  });
