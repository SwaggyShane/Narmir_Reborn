...
module.exports = function (db) {
    ...
    try {
      await db.withTransaction(async () => {
        ...
        for (const [unit, amount] of Object.entries(clean_alloc)) {
          if (amount > (k[unit] || 0)) {
            const err = new Error(`Not enough ${unit}`);
            err.statusCode = 400;
            throw err;
          }
        }
        if (allocValidation.total > capacity) {
          const err = new Error(`Exceeds training capacity (${capacity})`);
          err.statusCode = 400;
          throw err;
        }
  ...
  router.post('/build-allocation', requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      await db.withTransaction(async () => {
        const k = await db.get(
          'SELECT id, engineers, resource_build_allocation, training_allocation FROM kingdoms WHERE player_id = $1 FOR UPDATE',
          [req.player.playerId],
        );
        if (!k) {
          const err = new Error('Kingdom not found');
        ...
        if (allocValidation.total + resourceTotal + trainingTotal + engOnExpedition > k.engineers) {
          const err = new Error(`Not enough available engineers. Allocated: ${allocValidation.total.toLocaleString()} build, ${resourceTotal.toLocaleString()} resource, ${trainingTotal.toLocaleString()} training, ${engOnExpedition.toLocaleString()} on expeditions. Total available: ${k.engineers.toLocaleString()}`);
          err.statusCode = 400;
          throw err;
        }
  ...
  router.post('/resource-build-allocation', requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      await db.withTransaction(async () => {
        const k = await db.get(
          'SELECT id, engineers, build_allocation, training_allocation FROM kingdoms WHERE player_id = $1 FOR UPDATE',
          [req.player.playerId],
        );
        if (!k) {
          const err = new Error('Kingdom not found');
        ...
        if (allocValidation.total + buildTotal + trainingTotal + engOnExpedition > k.engineers) {
          const err = new Error(`Not enough available engineers. Allocated: ${allocValidation.total.toLocaleString()} resource, ${buildTotal.toLocaleString()} build, ${trainingTotal.toLocaleString()} training, ${engOnExpedition.toLocaleString()} on expeditions. Total available: ${k.engineers.toLocaleString()}`);
          err.statusCode = 400;
          throw err;
        }
      ...
      console.error('[resource-build-allocation] error:', err.message);
      const statusCode = err.statusCode || 500;
      res.status(statusCode).json({ error: err.message });
    }
  });
  ...
  router.post('/demolish', requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      const { updates, message } = await db.withTransaction(async () => {
  ...
  router.post('/build', requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      const { updates, message, buildTime, cost } = await db.withTransaction(async () => {
        const k = await db.get(`SELECT ${KINGDOM_RESOURCE} FROM kingdoms WHERE player_id = $1 FOR UPDATE`, [
          req.player.playerId,
        ]);
        if (!k) {
        ...
        if (!tier) {
          const err = new Error('Invalid building');
          err.statusCode = 400;
          throw err;
        }
        ...
        if (k.wood < cost.wood) {
          const err = new Error('Insufficient wood');
          err.statusCode = 400;
          throw err;
        }
        if (k.stone < cost.stone) {
          const err = new Error('Insufficient stone');
          err.statusCode = 400;
          throw err;
        }
        if (k.iron < cost.iron) {
          const err = new Error('Insufficient iron');
          err.statusCode = 400;
          throw err;
        }
  ...
  router.post('/smithy/buy-hammers', requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      const { bought, actualCost, newHammers, newGold } = await db.withTransaction(async () => {
        ...
        if (k.gold < cost) {
          const err = new Error(`Need ${cost.toLocaleString()} gold`);
          err.statusCode = 400;
          throw err;
        }
        ...
        if (bought <= 0) {
          const err = new Error('Hammer storage full');
          err.statusCode = 400;
          throw err;
        }
        ...
        return { bought, actualCost, newHammers, newGold: k.gold - actualCost };
      });
  ...
  router.post('/smithy/buy-scaffolding', requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      const { bought, actualCost, newScaff, newGold, hasSmithy } = await db.withTransaction(async () => {
        const k = await db.get(`SELECT ${KINGDOM_SMITHY} FROM kingdoms WHERE player_id=$1 FOR UPDATE`, [
          req.player.playerId,
        ]);
        if (!k) {
        ...
        if (bought <= 0) {
          const err = new Error('Scaffolding storage full');
          err.statusCode = 400;
          throw err;
        }
        ...
        return { bought, actualCost, newScaff, newGold: k.gold - actualCost, hasSmithy };
      });
  ...
  router.post('/tower-craft', requireAuth, requireCsrfToken, async (req, res) => {
    ...
    try {
      const { allocation } = await db.withTransaction(async () => {
        const k = await db.get(
        ...
        return { allocation: JSON.stringify(alloc) };
      });
