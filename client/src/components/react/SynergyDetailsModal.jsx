      ...
      <div className="rounded-xl border border-white/10 bg-gradient-to-br from-zinc-900 to-white/5 max-w-[800px] w-full max-h-[90vh] shadow-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-lg p-5">
            <h3 className="m-0 mb-4 text-sm font-semibold uppercase tracking-wider text-green">✦ Passive Ability (Always Active)</h3>
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-lg p-5">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="bg-white/[0.02] border border-white/5 rounded p-3 flex flex-col gap-2">
              ...
              {synergy.active?.benefit && (
                <div className="bg-green/[0.08] border border-green/20 rounded p-3">
                  <p className="m-0 mb-2 text-xs font-semibold text-green uppercase tracking-wider">Benefits:</p>
                  <div className="grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-2">
                    {Object.entries(synergy.active.benefit).map(([key, value]) => (
                      <div key={key} className="flex flex-col items-center p-3 bg-white/5 rounded border-l-4 border-green text-center">
                        <span className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-1">{formatKey(key)}</span>
                        <span className="text-base font-bold text-green">{formatValue(value, key)}</span>
                      </div>
                    ))}
                  </div>
                </div>
