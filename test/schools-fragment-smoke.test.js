...
function withSchoolFragment(fragmentName) {
  const FRAGMENTS = require('../game/world-fragment-bonuses');
  const config = FRAGMENTS[fragmentName]?.schools;
  if (!config) throw new Error(`No schools config for fragment: ${fragmentName}`);
  const bonuses = {
    schools: {
...
function test_attunements_noFragment() {
  ...
  const k = baseKingdom();
  const events = [];
  const updates = processSchoolAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no fragment → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}

function test_attunements_noSchools() {
  clearParseCache();
  const k = baseKingdom({ bld_schools: 0, fragment_bonuses: withSchoolFragment('Cursed Bloodstone') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no schools → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}
...
function test_attunements_passiveOnly_volcanicRock() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Volcanic Rock') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
function test_attunements_passiveOnly_ancientElvenWood() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Ancient Elven Wood') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
function test_attunements_passiveOnly_dragonScale() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Dragon Scale') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
function test_attunements_passiveOnly_abyssalCrystal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Abyssal Crystal') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
function test_attunements_passiveOnly_celestialFeather() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Celestial Feather') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
function test_attunements_passiveOnly_dwarvenStarMetal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Dwarven Star-Metal') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
function test_attunements_passiveOnly_tearsOfWorldTree() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Tears of the World Tree') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
function test_attunements_passiveOnly_titanBone() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withSchoolFragment('Titan Bone') });
  const events = [];
  const updates = processSchoolAttunements(k, events);
...
// ── Cursed Bloodstone: Taboo Alchemical Arts ─────────────────────────────────
...
function test_attunements_cursedBloodstone_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 80, fragment_bonuses: withSchoolFragment('Cursed Bloodstone') });
    const events = [];
    const updates = processSchoolAttunements(k, events);
...
function test_attunements_cursedBloodstone_trigger() {
  ...
  try {
    ...
    assert.ok(events.some(e => e.message.includes('Taboo Alchemical Arts')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_cursedBloodstone_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withSchoolFragment('Cursed Bloodstone'),
...
// ── Void Essence: Quantum Paradoxes ──────────────────────────────────────────
...
function test_attunements_voidEssence_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 70, fragment_bonuses: withSchoolFragment('Void Essence') });
    const events = [];
    const updates = processSchoolAttunements(k, events);
    assert.strictEqual(updates.active_effects, undefined, 'no penalty without trigger');
...
function test_attunements_voidEssence_trigger() {
  ...
  try {
    ...
    const ae = JSON.parse(updates.active_effects || '{}');
    assert.strictEqual(ae.fragment_happiness_penalty, -1, 'penalty stored in active_effects');
    assert.ok(events.some(e => e.message.includes('Quantum Paradoxes')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_voidEssence_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withSchoolFragment('Void Essence'),
      active_effects: '{"fragment_happiness_penalty": -2}',
    });
    const events = [];
    const updates = processSchoolAttunements(k, events);
