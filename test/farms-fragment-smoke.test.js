...
function withFarmFragment(fragmentName) {
  const FRAGMENTS = require('../game/world-fragment-bonuses');
  const config = FRAGMENTS[fragmentName]?.farms;
  if (!config) throw new Error(`No farms config for fragment: ${fragmentName}`);
  const bonuses = {
    farms: {
...
function test_attunements_noFragment() {
  ...
  const k = baseKingdom();
  const events = [];
  const updates = processFarmAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no fragment → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}

function test_attunements_noFarms() {
  clearParseCache();
  const k = baseKingdom({ bld_farms: 0, fragment_bonuses: withFarmFragment('Cursed Bloodstone') });
  const events = [];
  const updates = processFarmAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no farms → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}
...
function test_attunements_passiveOnly_volcanicRock() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Volcanic Rock') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
function test_attunements_passiveOnly_ancientElvenWood() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Ancient Elven Wood') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
function test_attunements_passiveOnly_dragonScale() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Dragon Scale') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
function test_attunements_passiveOnly_abyssalCrystal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Abyssal Crystal') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
function test_attunements_passiveOnly_celestialFeather() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Celestial Feather') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
function test_attunements_passiveOnly_dwarvenStarMetal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Dwarven Star-Metal') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
function test_attunements_passiveOnly_tearsOfWorldTree() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Tears of the World Tree') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
function test_attunements_passiveOnly_titanBone() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withFarmFragment('Titan Bone') });
  const events = [];
  const updates = processFarmAttunements(k, events);
...
// ── Cursed Bloodstone: Bloodsoaked Fields ────────────────────────────────────
...
function test_attunements_cursedBloodstone_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 80, fragment_bonuses: withFarmFragment('Cursed Bloodstone') });
    const events = [];
    const updates = processFarmAttunements(k, events);
...
function test_attunements_cursedBloodstone_trigger() {
  ...
  try {
    ...
    assert.ok(events.some(e => e.message.includes('Bloodsoaked Fields')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_cursedBloodstone_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withFarmFragment('Cursed Bloodstone'),
...
// ── Void Essence: Void Crops ─────────────────────────────────────────────────
// 20% outer trigger, then 50% bounty/failure branch

function test_attunements_voidEssence_noTrigger() {
  clearParseCache();
  const origRandom = Math.random;
  Math.random = () => 0.50; // > 0.20: outer roll fails
  try {
    const k = baseKingdom({ bld_farms: 5, food: 1000, fragment_bonuses: withFarmFragment('Void Essence') });
    const events = [];
    const updates = processFarmAttunements(k, events);
    assert.strictEqual(updates.food, undefined, 'no food change without trigger');
...
function test_attunements_voidEssence_triggerBounty() {
  clearParseCache();
  let callCount = 0;
  const origRandom = Math.random;
  // First call (outer): 0.10 < 0.20 → triggers; second call (branch): 0.20 < 0.50 → bounty
  Math.random = () => callCount++ === 0 ? 0.10 : 0.20;
  try {
    const k = baseKingdom({ bld_farms: 5, food: 1000, fragment_bonuses: withFarmFragment('Void Essence') });
    const events = [];
    const updates = processFarmAttunements(k, events);
    assert.strictEqual(updates.food, 1050, '5 farms × 10 food = +50 food surplus');
    assert.ok(events.some(e => e.message.includes('Void Crops') && e.message.includes('surplus')), 'bounty event fired');
  } finally {
    Math.random = origRandom;
  }
}

function test_attunements_voidEssence_triggerFailure() {
  clearParseCache();
  let callCount = 0;
  const origRandom = Math.random;
  // First call (outer): 0.10 < 0.20 → triggers; second call (branch): 0.80 >= 0.50 → failure
  Math.random = () => callCount++ === 0 ? 0.10 : 0.80;
  try {
    const k = baseKingdom({ bld_farms: 5, food: 1000, fragment_bonuses: withFarmFragment('Void Essence') });
    const events = [];
    const updates = processFarmAttunements(k, events);
    assert.strictEqual(updates.food, 950, '5 farms × 10 food = -50 food failure');
    assert.ok(events.some(e => e.message.includes('Void Crops') && e.message.includes('failure')), 'failure event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
const tests = [
  ...
  test_attunements_voidEssence_triggerBounty,
  test_attunements_voidEssence_triggerFailure,
];
