...
function withMausoleumFragment(fragmentName) {
  const FRAGMENTS = require('../game/world-fragment-bonuses');
  const config = FRAGMENTS[fragmentName]?.mausoleums;
  if (!config) throw new Error(`No mausoleums config for fragment: ${fragmentName}`);
  const bonuses = {
    mausoleums: {
...
function test_attunements_noFragment() {
  ...
  const k = baseKingdom();
  const events = [];
  const updates = processMausoleumAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no fragment → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}

function test_attunements_noMausoleums() {
  clearParseCache();
  const k = baseKingdom({ bld_mausoleums: 0, fragment_bonuses: withMausoleumFragment('Cursed Bloodstone') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no mausoleums → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}
...
function test_attunements_passiveOnly_volcanicRock() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Volcanic Rock') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_passiveOnly_ancientElvenWood() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Ancient Elven Wood') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_passiveOnly_dragonScale() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Dragon Scale') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_passiveOnly_abyssalCrystal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Abyssal Crystal') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_passiveOnly_celestialFeather() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Celestial Feather') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_passiveOnly_dwarvenStarMetal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Dwarven Star-Metal') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_passiveOnly_tearsOfWorldTree() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Tears of the World Tree') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_passiveOnly_titanBone() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withMausoleumFragment('Titan Bone') });
  const events = [];
  const updates = processMausoleumAttunements(k, events);
...
function test_attunements_cursedBloodstone_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 80, fragment_bonuses: withMausoleumFragment('Cursed Bloodstone') });
    const events = [];
    const updates = processMausoleumAttunements(k, events);
...
function test_attunements_cursedBloodstone_trigger() {
  ...
  try {
    ...
    assert.ok(events.some(e => e.message.includes('Cruor Coils')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_cursedBloodstone_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withMausoleumFragment('Cursed Bloodstone'),
...
// ── Void Essence: Shattered Portal Sarcophagi ───────────────────────────────
...
function test_attunements_voidEssence_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 70, fragment_bonuses: withMausoleumFragment('Void Essence') });
    const events = [];
    const updates = processMausoleumAttunements(k, events);
    assert.strictEqual(updates.active_effects, undefined, 'no penalty without trigger');
...
function test_attunements_voidEssence_trigger() {
  ...
  try {
    ...
    const ae = JSON.parse(updates.active_effects || '{}');
    assert.strictEqual(ae.fragment_happiness_penalty, -1, 'penalty stored in active_effects');
    assert.ok(events.some(e => e.message.includes('Shattered Portal Sarcophagi')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_voidEssence_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withMausoleumFragment('Void Essence'),
      active_effects: '{"fragment_happiness_penalty": -2}',
    });
    const events = [];
    const updates = processMausoleumAttunements(k, events);
