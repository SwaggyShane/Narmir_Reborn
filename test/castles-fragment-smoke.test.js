...
function withCastleFragment(fragmentName) {
  const FRAGMENTS = require('../game/world-fragment-bonuses');
  const config = FRAGMENTS[fragmentName]?.castles;
  if (!config) throw new Error(`No castles config for fragment: ${fragmentName}`);
  const bonuses = {
    castles: {
...
function test_attunements_noFragment() {
  ...
  const k = baseKingdom();
  const events = [];
  const updates = processCastleAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no fragment → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}

function test_attunements_noCastles() {
  clearParseCache();
  const k = baseKingdom({ bld_castles: 0, fragment_bonuses: withCastleFragment('Cursed Bloodstone') });
  const events = [];
  const updates = processCastleAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no castles → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}
...
function test_attunements_passiveOnly_volcanicRock() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withCastleFragment('Volcanic Rock') });
  const events = [];
  const updates = processCastleAttunements(k, events);
...
function test_attunements_passiveOnly_ancientElvenWood() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withCastleFragment('Ancient Elven Wood') });
  const events = [];
  const updates = processCastleAttunements(k, events);
...
function test_attunements_passiveOnly_dragonScale() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withCastleFragment('Dragon Scale') });
  const events = [];
  const updates = processCastleAttunements(k, events);
...
function test_attunements_passiveOnly_abyssalCrystal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withCastleFragment('Abyssal Crystal') });
  const events = [];
  const updates = processCastleAttunements(k, events);
...
function test_attunements_passiveOnly_celestialFeather() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withCastleFragment('Celestial Feather') });
  const events = [];
  const updates = processCastleAttunements(k, events);
...
function test_attunements_passiveOnly_dwarvenStarMetal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withCastleFragment('Dwarven Star-Metal') });
  const events = [];
  const updates = processCastleAttunements(k, events);
...
function test_attunements_passiveOnly_titanBone() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withCastleFragment('Titan Bone') });
  const events = [];
  const updates = processCastleAttunements(k, events);
...
// ── Tears of the World Tree: Elder Sap Tapestries ───────────────────────────

function test_attunements_tearsOfWorldTree_goldGain() {
  clearParseCache();
  const k = baseKingdom({ bld_castles: 5, gold: 10000, fragment_bonuses: withCastleFragment('Tears of the World Tree') });
  const events = [];
  const updates = processCastleAttunements(k, events);
  assert.strictEqual(updates.gold, 10015, '5 castles × 3 gold = +15 gold');
  assert.ok(events.some(e => e.message.includes('Elder Sap Tapestries')), 'event fired');
}

function test_attunements_tearsOfWorldTree_scales() {
  clearParseCache();
  const k = baseKingdom({ bld_castles: 10, gold: 5000, fragment_bonuses: withCastleFragment('Tears of the World Tree') });
  const events = [];
  const updates = processCastleAttunements(k, events);
  assert.strictEqual(updates.gold, 5030, '10 castles × 3 gold = +30 gold');
}

// ── Cursed Bloodstone: Blood-Sacrifice Vaults ────────────────────────────────
...
function test_attunements_cursedBloodstone_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 80, fragment_bonuses: withCastleFragment('Cursed Bloodstone') });
    const events = [];
    const updates = processCastleAttunements(k, events);
...
function test_attunements_cursedBloodstone_trigger() {
  ...
  try {
    ...
    assert.ok(events.some(e => e.message.includes('Blood-Sacrifice Vaults')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_cursedBloodstone_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withCastleFragment('Cursed Bloodstone'),
...
// ── Void Essence: Astral Phasing Throne ─────────────────────────────────────
...
function test_attunements_voidEssence_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 70, fragment_bonuses: withCastleFragment('Void Essence') });
    const events = [];
    const updates = processCastleAttunements(k, events);
    assert.strictEqual(updates.active_effects, undefined, 'no penalty without trigger');
...
function test_attunements_voidEssence_trigger() {
  ...
  try {
    ...
    const ae = JSON.parse(updates.active_effects || '{}');
    assert.strictEqual(ae.fragment_happiness_penalty, -1, 'penalty stored in active_effects');
    assert.ok(events.some(e => e.message.includes('Astral Phasing Throne')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_voidEssence_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withCastleFragment('Void Essence'),
      active_effects: '{"fragment_happiness_penalty": -2}',
    });
    const events = [];
    const updates = processCastleAttunements(k, events);
