...
function withLibraryFragment(fragmentName) {
  const FRAGMENTS = require('../game/world-fragment-bonuses');
  const config = FRAGMENTS[fragmentName]?.libraries;
  if (!config) throw new Error(`No libraries config for fragment: ${fragmentName}`);
  const bonuses = {
    libraries: {
...
function test_attunements_noFragment() {
  ...
  const k = baseKingdom();
  const events = [];
  const updates = processLibraryAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no fragment → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}

function test_attunements_noLibraries() {
  clearParseCache();
  const k = baseKingdom({ bld_libraries: 0, fragment_bonuses: withLibraryFragment('Cursed Bloodstone') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no libraries → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}
...
function test_attunements_passiveOnly_volcanicRock() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Volcanic Rock') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
function test_attunements_passiveOnly_ancientElvenWood() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Ancient Elven Wood') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
function test_attunements_passiveOnly_dragonScale() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Dragon Scale') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
function test_attunements_passiveOnly_abyssalCrystal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Abyssal Crystal') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
function test_attunements_passiveOnly_celestialFeather() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Celestial Feather') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
function test_attunements_passiveOnly_dwarvenStarMetal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Dwarven Star-Metal') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
function test_attunements_passiveOnly_tearsOfWorldTree() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Tears of the World Tree') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
function test_attunements_passiveOnly_titanBone() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withLibraryFragment('Titan Bone') });
  const events = [];
  const updates = processLibraryAttunements(k, events);
...
// ── Cursed Bloodstone: Sanguine Cartography ─────────────────────────────────
...
function test_attunements_cursedBloodstone_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 80, fragment_bonuses: withLibraryFragment('Cursed Bloodstone') });
    const events = [];
    const updates = processLibraryAttunements(k, events);
...
function test_attunements_cursedBloodstone_trigger() {
  ...
  try {
    ...
    assert.ok(events.some(e => e.message.includes('Sanguine Cartography')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_cursedBloodstone_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withLibraryFragment('Cursed Bloodstone'),
...
function test_attunements_voidEssence_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 70, fragment_bonuses: withLibraryFragment('Void Essence') });
    const events = [];
    const updates = processLibraryAttunements(k, events);
    assert.strictEqual(updates.active_effects, undefined, 'no penalty without trigger');
...
function test_attunements_voidEssence_trigger() {
  ...
  try {
    ...
    const ae = JSON.parse(updates.active_effects || '{}');
    assert.strictEqual(ae.fragment_happiness_penalty, -1, 'penalty stored in active_effects');
    assert.ok(events.some(e => e.message.includes('Void Codex')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_voidEssence_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withLibraryFragment('Void Essence'),
      active_effects: '{"fragment_happiness_penalty": -2}',
    });
    const events = [];
    const updates = processLibraryAttunements(k, events);
