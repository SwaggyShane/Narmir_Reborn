...
function withOutpostFragment(fragmentName) {
  const FRAGMENTS = require('../game/world-fragment-bonuses');
  const config = FRAGMENTS[fragmentName]?.outposts;
  if (!config) throw new Error(`No outposts config for fragment: ${fragmentName}`);
  const bonuses = {
    outposts: {
...
function test_attunements_noOutposts() {
  clearParseCache();
  const k = baseKingdom({ bld_outposts: 0, fragment_bonuses: withOutpostFragment('Cursed Bloodstone') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no outposts → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}

// ── Passive-only fragments (all 9 non-Cursed-Bloodstone) ─────────────────────

function test_attunements_passiveOnly_volcanicRock() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Volcanic Rock') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_ancientElvenWood() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Ancient Elven Wood') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_dragonScale() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Dragon Scale') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_abyssalCrystal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Abyssal Crystal') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_celestialFeather() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Celestial Feather') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_dwarvenStarMetal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Dwarven Star-Metal') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_tearsOfWorldTree() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Tears of the World Tree') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_voidEssence() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Void Essence') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
function test_attunements_passiveOnly_titanBone() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withOutpostFragment('Titan Bone') });
  const events = [];
  const updates = processOutpostAttunements(k, events);
...
// ── Cursed Bloodstone: Sanguine Warning Totems ───────────────────────────────
