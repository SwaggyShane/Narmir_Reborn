...
function withHousingFragment(fragmentName) {
  const FRAGMENTS = require('../game/world-fragment-bonuses');
  const config = FRAGMENTS[fragmentName]?.housing;
  if (!config) throw new Error(`No housing config for fragment: ${fragmentName}`);
  const bonuses = {
    housing: {
...
function test_attunements_noFragment() {
  ...
  const k = baseKingdom();
  const events = [];
  const updates = processHousingAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no fragment → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}

function test_attunements_noHousing() {
  clearParseCache();
  const k = baseKingdom({ bld_housing: 0, fragment_bonuses: withHousingFragment('Cursed Bloodstone') });
  const events = [];
  const updates = processHousingAttunements(k, events);
  assert.deepStrictEqual(updates, {}, 'no housing → empty updates');
  assert.strictEqual(events.length, 0, 'no events');
}
...
function test_attunements_passiveOnly_volcanicRock() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Volcanic Rock') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
function test_attunements_passiveOnly_ancientElvenWood() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Ancient Elven Wood') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
function test_attunements_passiveOnly_dragonScale() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Dragon Scale') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
function test_attunements_passiveOnly_abyssalCrystal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Abyssal Crystal') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
function test_attunements_passiveOnly_celestialFeather() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Celestial Feather') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
function test_attunements_passiveOnly_dwarvenStarMetal() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Dwarven Star-Metal') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
function test_attunements_passiveOnly_tearsOfWorldTree() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Tears of the World Tree') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
function test_attunements_passiveOnly_titanBone() {
  clearParseCache();
  const k = baseKingdom({ fragment_bonuses: withHousingFragment('Titan Bone') });
  const events = [];
  const updates = processHousingAttunements(k, events);
...
// ── Cursed Bloodstone: Blood Pact Lodgings ───────────────────────────────────
...
function test_attunements_cursedBloodstone_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 80, fragment_bonuses: withHousingFragment('Cursed Bloodstone') });
    const events = [];
    const updates = processHousingAttunements(k, events);
...
function test_attunements_cursedBloodstone_trigger() {
  ...
  try {
    ...
    assert.ok(events.some(e => e.message.includes('Blood Pact Lodgings')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_cursedBloodstone_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withHousingFragment('Cursed Bloodstone'),
...
// ── Void Essence: Void Pocket Lofts ─────────────────────────────────────────
...
function test_attunements_voidEssence_noTrigger() {
  ...
  try {
    const k = baseKingdom({ happiness: 70, fragment_bonuses: withHousingFragment('Void Essence') });
    const events = [];
    const updates = processHousingAttunements(k, events);
    assert.strictEqual(updates.active_effects, undefined, 'no penalty without trigger');
...
function test_attunements_voidEssence_trigger() {
  ...
  try {
    ...
    const ae = JSON.parse(updates.active_effects || '{}');
    assert.strictEqual(ae.fragment_happiness_penalty, -1, 'penalty stored in active_effects');
    assert.ok(events.some(e => e.message.includes('Void Pocket Lofts')), 'event fired');
  } finally {
    Math.random = origRandom;
  }
}
...
function test_attunements_voidEssence_stacks() {
  ...
  try {
    const k = baseKingdom({
      fragment_bonuses: withHousingFragment('Void Essence'),
      active_effects: '{"fragment_happiness_penalty": -2}',
    });
    const events = [];
    const updates = processHousingAttunements(k, events);
