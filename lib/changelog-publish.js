...
const { nowUnix } = require('../game/lib/timestamp');
...
async function publishChangelogEntry(db, {
  ...
  const insert = await db.run(
    `INSERT INTO changelog_entries (title, description, body_md, category, source, source_id, author_name, created_at)
  ...
  if (discordSent && entryId) {
    await db.run('UPDATE changelog_entries SET discord_sent = 1 WHERE id = $1', [entryId]);
  }
