...
// HTTPS redirect (prod only). ALLOW_INSECURE_HTTP opts out for local-network
// testing (e.g. a production-mode build served over plain HTTP to a phone on
// the same Wi-Fi, with no local TLS cert) — off by default, never set on
// Railway, so real production behavior is unchanged.
