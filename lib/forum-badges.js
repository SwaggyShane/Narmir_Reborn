...
async function getPlayerForumStats(db, playerId) {
  ...
  const isMod = await db.get(`SELECT 1 FROM forum_moderators WHERE player_id = ? LIMIT 1`, [playerId]);
  const player = await db.get(`SELECT is_admin FROM players WHERE id = ?`, [playerId]);
