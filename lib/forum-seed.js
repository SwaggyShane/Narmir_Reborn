...
async function seedForumStructure(db) {
  ...
  for (const category of FORUM_CATEGORIES) {
    for (const board of category.boards) {
      const existing = await db.get(`SELECT id, category_key FROM forum_boards WHERE name = ?`, [board.name]);
      if (existing) {
        await db.run(
          `UPDATE forum_boards
           SET description = ?, order_index = ?, category_key = ?, category_label = ?,
               category_order = ?, is_active = 1, updated_at = ?
           WHERE id = ?`,
  ...
  for (const legacyName of LEGACY_BOARD_NAMES) {
    ...
    await db.run(
      `UPDATE forum_boards SET is_active = 0, updated_at = ?
       WHERE name = ? AND (category_key IS NULL OR category_key = '')`,
      [now, legacyName],
    );
  }
  ...
  if (dupGeneral.length > 1) {
    for (const row of dupGeneral.slice(0, -1)) {
      if (!row.category_key) {
        await db.run(`UPDATE forum_boards SET is_active = 0, name = ?, updated_at = ? WHERE id = ?`, [
