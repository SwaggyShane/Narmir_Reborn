/**
 * Discord notifications from the game server and shared embed builders for discord-bot.js.
 * Bug reports: webhook (optional) or bot token + channel ID on the web service.
 * If the web service has no Discord config, discord-bot.js polls unsent rows and posts to #bug-reports.
 */
...
async function postBugReportToDiscord(report) {
  try {
    const embed = buildBugReportEmbed(report);
...
module.exports = { postDiscordEmbed, postBugReportToDiscord, buildBugReportEmbed };