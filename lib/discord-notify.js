...
async function postEmbedToChannel({ channelIdEnv, webhookEnv, embed }) {
  ...
  const webhook = webhookEnv ? trimEnv(process.env[webhookEnv]) : null;
  if (webhook) {
  ...
  const token = trimEnv(process.env.DISCORD_BOT_TOKEN);
  const channelId = trimEnv(process.env[channelIdEnv]);
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (parsed?.code === 50013) {
        detail = `Missing Permissions — bot cannot post to channel ${channelId}. Check #channel Permissions for this bot role (Send Messages + Embed Links).`;
      } else if (parsed?.message) {
        detail = `${parsed.message} (code ${parsed.code ?? 'n/a'})`;
      }
    } catch { /* keep raw body */ }
    console.warn(`[discord-notify] Channel post (${channelIdEnv}=${channelId}) failed:`, res.status, detail);
    return false;
  }
  return true;
}
...
/**
 * @param {{ username?: string, kingdomName?: string, category?: string, message: string, contextPanel?: string, pageUrl?: string, consoleLog?: string, reportId?: number }} report
 */
function buildBugReportEmbed(report) {
  ...
  const consolePreview = truncateTail(String(report.consoleLog || '').replace(/```/g, "'''"), 880);
  if (consolePreview) {
    fields.push({
      name: 'Console (recent)',
      value: `\`\`\`\n${consolePreview}\n\`\`\``,
