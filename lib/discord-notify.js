...
/**
 * @param {{ category?: string, description: string, wishlistId?: number }} entry
 */
function buildChangelogEmbed(entry) {
  const category = entry.category ? String(entry.category).replace(/_/g, ' ') : 'update';
...
/**
 * Wishlist item completed (changelog entry) → #updates
 */
