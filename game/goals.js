...
const WEEKLY_GOALS = [
  ...
  { id: 'expedition_started', label: 'Send Expeditions', min: 10, max: 25, prizeStr: 'world_fragment', prizeType: 'world_fragment', prizeMult: 1 },
...
function generateGoals(k) {
  let goals = { daily: { expiresAt: 0, goals: [] }, weekly: { expiresAt: 0, goals: [] } };
  try {
  ...
  if (now > goals.weekly.expiresAt) {
    ...
    for(let i=0; i<7; i++) {
       ...
       goals.weekly.goals.push({
         ...
         claimed: false,
         prizeType: def.prizeType,
         prizeAmount: def.prizeType === 'world_fragment' || def.prizeType === 'war_machines' ? rand(1, 2) : Math.max(1, Math.floor(target * def.prizeMult * (roll(0.5) ? 1.5 : 1)))
       });
    }
    updated = true;
  }
...
function progressGoal(k, updates, type, amount) {
  ...
  [goals.daily, goals.weekly].forEach(period => {
...
function claimGoal(k, updates, events, groupId, goalId) {
   ...
   if (goal.prizeType === 'world_fragment') {
     ...
     events.push({ type: 'system', message: `Goal fulfilled: Obtained World Fragment (${frag})!` });
     return { success: true, message: `Claimed ${frag}!` };
   } else {
