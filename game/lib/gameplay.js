...
function expeditionRewards(type, rangers, fighters, k, db, originalRewards) {
  ...
  if (type === "scout") {
    ...
    // Every caller (engine.js's async resolveExpeditions loop, formerly also
    // resolveInstantExpedition) adds survivors back via this increment, the
    // same way every other expedition type reports returns. An absolute
    // `updates.rangers` used to also be set here, which double-subtracted
    // totalLost for any kingdom with more total rangers than it sent (k here
    // already excludes the sent troops, deducted at launch) — removed rather
    // than requiring every caller to remember to strip it.
