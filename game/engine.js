  ...
  try {
    ...
    if (discoveries && discoveries.length > 0) {
      ...
      if (newKingdoms.length > 0) {
        ...
        const otherKingdoms = await db.all('SELECT id, race, kingdom_name FROM kingdoms WHERE id != $1', [kingdom.id]);
        ...
        for (const discovered of newKingdoms) {
          ...
          if (match && !disc[match.id]) {
            disc[match.id] = { found: true, discovered_turn: discovered.discovered_turn, name: match.kingdom_name };
            discoveredCount++;
          }
        }
