...
- [x] Announcement (POST /announce — Socket.io global broadcast)
- [x] Chat mod promote/demote (GET /chat-mods, POST /chat-mod)
- [x] Chat ban list + unban (GET /chat-bans, POST /chat-unban)
- [x] Promote to admin (POST /promote) with confirm dialog
- [x] Test kingdom provision (POST /test-kingdoms/setup) with results table
- [x] Bulk actions: Reset All Turns, Reset All Kingdoms, Flush Locations, Flush Support Troops
- [x] AI hiatus toggle (duplicate of KingdomsPanel control for manage-tab convenience)
...
### Phase 6a — Soft cutover ✅ MERGED (PR #589)

**Branch:** `admin-soft-six-prep` → merged to `main`

**Status:** React admin is the default at `/admin`. Legacy fallback remains at `/admin?legacy=1`.

- [x] P0 parity: `fortified_blueprints` in kingdom editor (scalar INTEGER — matches schema + legacy)
- [x] P0 parity: Spells reference sub-tab (Detailed Lists)
- [x] P1 parity: Evolution changelog sub-tab (completed wishes)
- [x] P2 parity: Admin notes localStorage migration (with `Array.isArray` guard)
...
5. **Phase 5 PR** — MERGED (PR #588). ConfigPanel, SoundsPanel, PrestigePanel, FragmentsPanel, SecurityPanel wired into AdminShell. All tabs live.
