...
**Files**
- `game/combat-new.js` - Advanced combat system with individual troop HP and injury states
- `game/combat-resolver.js` - Combat execution engine

**Status**: Complete and protected. The advanced path exists locally behind `USE_COMBAT_V2=1` and must not be deleted or treated as orphaned. The legacy combat path remains the default unless the flag is set.

**Importance**: This is a major system overhaul that affects core game mechanics. It must stay intact and documented correctly.

**Plan**: Keep the feature-flagged advanced path, diagnostics, and docs aligned with the current codebase. Do not push to remote unless asked.
...
The advanced adapter is already present locally behind a feature flag. Next work is cleanup, balance review, and documentation alignment. Do not use legacy balance reports as final combat evidence.
